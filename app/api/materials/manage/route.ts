...
import { currentDocument, saveDocument, MATERIALS_20_DOC, MATERIALS_21_DOC } from '../../../document-store';
import { parseMaterials, addMaterial, removeMaterial } from '../../../materials-data';

function docIdFor(target: string | null) {
  return target === '21' ? MATERIALS_21_DOC : MATERIALS_20_DOC;
}

export async function GET(request: Request) {
  if ((await readRole()) !== 'admin') return Response.json({ error: '管理者のみ利用できます' }, { status: 403 });
  const docId = docIdFor(new URL(request.url).searchParams.get('target'));
...
export async function POST(request: Request) {
  if ((await readRole()) !== 'admin') return Response.json({ error: '管理者のみ利用できます' }, { status: 403 });
  const { section, label, url, target } = (await request.json()) as { section?: string; label?: string; url?: string; target?: string };
  if (!section?.trim() || !label?.trim() || !url?.trim()) return Response.json({ error: '区分・タイトル・URLを入力してください' }, { status: 400 });
  const docId = docIdFor(target ?? null);
  const html = await currentDocument(request, docId);
  const updated = addMaterial(html, { section: section.trim(), label: label.trim(), url: url.trim() });
  await saveDocument(updated, `資料「${label.trim()}」を追加しました`, docId);
  return Response.json({ ok: true });
}

export async function DELETE(request: Request) {
  if ((await readRole()) !== 'admin') return Response.json({ error: '管理者のみ利用できます' }, { status: 403 });
  const { label, url, target } = (await request.json()) as { label?: string; url?: string; target?: string };
  if (!label || !url) return Response.json({ error: '削除対象を指定してください' }, { status: 400 });
  const docId = docIdFor(target ?? null);
  const html = await currentDocument(request, docId);
  const updated = removeMaterial(html, url, label);
  await saveDocument(updated, `資料「${label}」を削除しました`, docId);
  return Response.json({ ok: true });
}
