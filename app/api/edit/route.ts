...
function structurallySane(html:string){for(const t of['h1','h2','ul','table','tr','td']){const opens=(html.match(new RegExp(`<${t}[ >]`,'gi'))||[]).length;const closes=(html.match(new RegExp(`</${t}>`,'gi'))||[]).length;if(opens!==closes)return false;}if(/<a\b(?=[^>]*\bhref=)[^>]*>\s*<\/a>/i.test(html))return false;if(/<li>\s*<\/li>/i.test(html))return false;return true;}
export async function POST(request:Request){
  ...
  if(!structurallySane(updated))return Response.json({error:'この変更を適用すると文書の構造（表の枠など）が壊れるため、保存を中止しました。対象箇所をもう少し具体的に指定するか、別の言い回しで試してください。'},{status:422});
