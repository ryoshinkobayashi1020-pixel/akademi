import { readRole } from '../../auth'; import { currentDocument, saveDocument, MAIN_DOC, MATERIALS_DOC } from '../../document-store'; import { parseMaterials } from '../../materials-data';
...
export async function POST(request:Request){
  ...
  if(docId===MAIN_DOC){
    const materialsHtml=await currentDocument(request,MATERIALS_DOC);
    const materials=parseMaterials(materialsHtml);
    if(materials.length)materialsContext=`\n\n参考資料一覧（このプロジェクトに登録済みの資料。タイトルとURLのみで本文までは読めないので、事実関係の裏付けではなくタイトルからの推測・言及・リンク付けの参考として使ってください）:\n${materials.map(m=>`- [${m.section}] ${m.label}: ${m.url}`).join('\n')}`;
  }
  const devPrompt='You edit a Japanese business-plan document (or its materials-list page). Return exact visible-text replacements. Each find must be copied verbatim (character-for-character, including spacing) from the supplied document, be unique, and under 180 characters. Preserve facts unless instructed. Use multiple operations when needed. If the instruction asks to add a hyperlink, set replace to HTML like <a href="URL" target="_blank">LABEL</a> — inserted right after the matched find text. Choosing the URL: if the instruction names or clearly points to one specific material from the supplied list, use that material\'s exact URL. If the instruction refers to the materials list/page in general (e.g. "資料一覧", "参考資料のページ", "資料一覧ページ") rather than one specific item, use /api/materials. If the instruction gives an explicit URL, use that. Never guess or substitute an unrelated material\'s URL — if you cannot determine a specific URL with confidence, link to /api/materials instead. A list of the project’s registered reference materials (title + URL only) may be supplied for context; you cannot read their actual file content.';
